    ...
    try {
        ...
        if (!stickerMessage) {
            await sock.sendMessage(chatId, {
                text: '⚠️ Balas pesan stiker dengan perintah *.simage* atau *.svideo* untuk mengonversi.'
            });
            return;
        }
        ...
        if (command === 'simage' || command === '.simage') {
            await convertToImage(sock, chatId, stickerFilePath, isAnimated);
        } else if (command === 'svideo' || command === '.svideo') {
            if (!isAnimated) {
                await sock.sendMessage(chatId, {
                    text: '❌ Stiker ini tidak animasi (GIF). Gunakan *.simage* untuk stiker biasa.'
                });
            ...
            await sock.sendMessage(chatId, {
                text: '❌ Perintah tidak valid. Gunakan:\n*.simage* - Konversi ke gambar\n*.svideo* - Konversi ke video (untuk stiker GIF)'
            });
            scheduleFileDeletion(stickerFilePath);
        }
